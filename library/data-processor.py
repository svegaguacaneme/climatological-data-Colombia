...
dir = {
    ...
    'Procesados': 'data',
    'Catalogo': 'library/IDEAM-catalog.csv',
    'Resultados' : 'output'
    }
...
def output(): # Crear excel con los resultados y abrirlo
    ...
    df_crudos.to_csv(os.path.join(dir.get('Resultados'), 'raw-data' + '.csv'), index=False, header=True, encoding='utf-8-sig', sep=",")

    df_procesados.to_csv(os.path.join(dir.get('Resultados'), 'processed-data' + '.csv'), index=False, header=True, encoding='utf-8-sig', sep=",")
